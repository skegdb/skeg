...
const CORPUS: &str = "skeg/bench-compare/embeddings_cache/corpus_mxbai-wiki-chunked_1m.npy";
const QUERY: &str = "skeg/bench-compare/embeddings_cache/queries_mxbai-wiki-chunked_1000.npy";
const K: usize = 10;
const PAD: usize = 1024; // mxbai, already a multiple of 8
...
fn main() {
    ...
    let dir: PathBuf = std::env::var("SKEG_STUDY_DIR")
        ...
        .join(format!("mxbai1m_n{n}"));
    let idx = build(&dir, &corpus, n);

    // Filter mode: "corr" = content-CORRELATED (matching set = a semantic cluster,
    // the N corpus vectors nearest a fixed center; real queries mostly sit OUTSIDE
    // it, the hard case for the walk). Default = "mod" (id%step, uniform/scattered).
    let corr = std::env::var("SKEG_FILTER").as_deref() == Ok("corr");
    // For correlated: rank all ids by cosine to a fixed center once, slice per size.
    let by_center: Vec<u64> = if corr {
        let center = &corpus[42];
        let mut s: Vec<(f32, u64)> = corpus
            .par_iter()
            .enumerate()
            .map(|(i, v)| (cosine_f32(center, v), i as u64))
            .collect();
    ...
    for &target in &TARGET_MATCHES {
        ...
        let mut ids: Vec<u64> = if corr {
            by_center[..target.min(by_center.len())].to_vec()
        } else {
        ...
        ids.sort_unstable();
        let matchn = ids.len();
        let set: AHashSet<u64> = ids.iter().copied().collect();
        let matches = |id: u64| set.contains(&id);
        // Truth: brute top-k over the matching set (gather its vectors).
