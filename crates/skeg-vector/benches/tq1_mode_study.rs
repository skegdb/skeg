...
//! Big recall-vs-latency study to decide the tq1 defaults: is hybrid worth it,
//! and is the k*8 rerank bump worth its latency? Forces the proxy mode via
//! SKEG_TQ1_MODE=pop|hybrid|asym (unset = auto) so all three can be measured on
//! ONE built index, and sweeps the rerank budget. Real DiskVamana path,
//! embeddings up to mxbai (dim <= 1024), large N, p50 + mean latency.
//!
//! Indexes are built once and cached under SKEG_STUDY_DIR (default: a temp dir)
//! so the three mode runs reuse the same graph instead of rebuilding. Run:
//!   for m in asym hybrid pop; do SKEG_TQ1_MODE=$m cargo bench ... tq1_mode_study; done
...
fn run(
    ...
    // Open the cached index, or build it once and persist.
    let dir = study_dir().join(format!("{label}_n{n}_d{pad}"));
    let idx = match DiskVamanaIndex::open_with_tier(&dir, QuantKind::TurboQuant { bits: 1 }) {
        Ok(i) if i.len() == n => i,
        _ => {
    ...
    for &rr in &RERANKS {
        ...
        let p50 = lats[nq / 2];
        let mean = lats.iter().sum::<f64>() / nq as f64;
        let recall = hits as f64 / (nq * K) as f64;
        println!(
            "  {label:<10} dim {pad:<4} n {n:<7} mode {mode:<7} rr{rr:<3}  recall {recall:.4}  p50 {p50:>5.0}us  mean {mean:>5.0}us"
        );
    }
}

fn main() {
    let n_cap = env("SKEG_BENCH_N", 100_000);
    let nq_cap = env("SKEG_NQ", 200);
    println!("=====================================================================");
    println!(
        "tq1 mode study (SKEG_TQ1_MODE={})  L_search={L_SEARCH} k={K} N<={n_cap} nq<={nq_cap}",
        std::env::var("SKEG_TQ1_MODE").unwrap_or_else(|_| "auto".into())
    );
    println!("cache: {}", study_dir().display());
    println!("=====================================================================");
