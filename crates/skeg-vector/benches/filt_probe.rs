...
fn main() {
    ...
    // Filter mode: "corr" = content-correlated (matching set = the N vectors
    // nearest a fixed center = a semantic cluster; real queries mostly sit
    // OUTSIDE it, the hard case for the walk). Default = uniform id%step.
    ...
    println!(
        "== tier tq{bits} | filter {} ==",
        if corr { "CORRELATED" } else { "uniform" }
    );

    for &sel in &[0.01_f64, 0.025, 0.05, 0.10, 0.20] {
        let matchn = (n as f64 * sel) as usize;
        let mut ids: Vec<u64> = if corr {
            by_center[..matchn.min(by_center.len())].to_vec()
        } else {
        ...
        // WALK (best budget) vs QSCAN (proxy scan + f32 rerank).
        row("walk L1500", &|q| {
