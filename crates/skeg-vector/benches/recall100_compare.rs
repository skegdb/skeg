...
fn main() {
    ...
    let corpus_path = std::env::var("SKEG_CORPUS")
        .unwrap_or_else(|_| format!("{ROOT}/skeg/bench-compare/embeddings_cache/corpus_mxbai-wiki.npy"));
    let query_path = std::env::var("SKEG_QUERY")
        .unwrap_or_else(|_| format!("{ROOT}/skeg/bench-compare/embeddings_cache/queries_mxbai-wiki_200.npy"));
    ...
    let truth: Vec<AHashSet<u64>> = queries
        ...
        .collect();
    println!("recall@100 (real, k=100): {n} x {dim}, {} queries", queries.len());
    ...
    for bits in [1u8, 2] {
        ...
        for &(label, ls, rr) in &[("default", 300usize, 800usize), ("wide", 2000, 12800)] {
            ...
            println!("  tq{bits} {label:<8} (l={ls},rr={rr})  recall@100 {:.4}  {ms:.2} ms/q", hits as f64 / (queries.len() * 100) as f64);
        }
        drop(idx);
        let _ = std::fs::remove_dir_all(&tmp);
    }
