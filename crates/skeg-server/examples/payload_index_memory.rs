...
use skeg_server::payload::{PayloadIndex, parse_fields};
...
fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("directory holding one vindex");
    let mut files = vec![];
    ...
    if std::env::var("DISK_ONLY").is_ok() {
        ...
        println!(
            "  from disk: +{:6.1} MB   {:5.1} B/vector   (file {:.1} MB)",
            dm / 1024.0,
            dm * 1024.0 / disk.len() as f64,
            file as f64 / 1e6
        );
    ...
    println!(
        "  in memory: +{:6.0} MB   {:5.0} B/vector   ({} ids covered)",
        mem / 1024.0,
        mem * 1024.0 / idx.len() as f64,
        idx.len()
    );
