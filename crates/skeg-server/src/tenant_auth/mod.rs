//! The authenticated multi-tenant profile of the RESP3 server.
//!
//! Implements [`TenantBackend`](crate::TenantBackend) on top of the
//! `skeg-tenant` primitives (auth store, tenant ids, argon2 password
//! hashing), plus a persisted per-tenant quota store an admin writes via
//! `SKEG.QUOTA.SET`.
//!
//! It lives here, behind the `tenant-auth` feature, rather than in a separate
//! crate with its own binary: one executable per protocol is the product
//! decision, so an operator cannot start the wrong one. `skeg-server-tenant`
//! remains as a compatibility shim that re-exports this module.
...
use crate::{AnonymousPolicy, QuotaAdminError, TenantBackend, TenantId, TenantLimits};
